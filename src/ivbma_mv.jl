...
B_Σ(σ_y_x, Σ_yx, Σ_xx) = I + Σ_yx * Σ_yx' * inv(Σ_xx) / σ_y_x
...
function post_sample_outcome(y_tilde, X, A, U, σ_y_x)
    n = size(X, 1); l = sixe(X, 2)
    ι = ones(length(n))
    M_ι = I - ι * inv(ι'ι) * ι'

    β_tilde = rand(MvNormal(A * U' * M_ι * y_tilde, σ_y_x * A))
...
function marginal_likelihood_outcome(y_tilde, A, U, σ_y_x, g)
    ...
    ml = (1/2) * (log(det(A)) - log(det(g * inv(U'U)))) + y_tilde' * M_ι * (I - U * A * U') * M_ι * y_tilde
    return ml 
end


function post_sample_treatment(X_tilde, B, V, Σ_xx, g)
    n = size(X_tilde, 1); l = sixe(X_tilde, 2)
    V_t_V_inv = inv(V'V)
    ι = ones(n)
    sf = g / (g + 1) # shrinkage factor

    Γ = rand(MvNormal(ι' * X_tilde / n, (1/n) * inv(B) * Σ_xx))
    Δ = rand(MatrixNormal(sf * V_t_V_inv * V'X_tilde, sf * V_t_V_inv, inv(B) * Σ_xx))
...
function ivbma_mv_mcmc(
    ...
    Σ_samples = Array{Diagonal{Float64, Vector{Float64}}}(undef, nsave)
    ...
    for i in 1:iter
        ...
        acc = min(1, exp(
            marginal_likelihood_outcome(y_tilde, A_prop, U_prop, σ_yx, g_L) - marginal_likelihood_outcome(y_tilde, A, U, σ_yx, g_L)
        ))
        if rand() < acc
            L = prop
            U = U_prop
        ...
        acc = min(1, exp(
            marginal_likelihood_treatment(X_tilde, B, V_prop, σ_yx, g_M) - marginal_likelihood_outcome(X_tilde, B, V, σ_yx, g_M)
        ))
        if rand() < acc
            M = prop
            V = V_prop
